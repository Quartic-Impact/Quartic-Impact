...
String.prototype.toPath = function (a, b) {
  return a + this.replace(/[.]/g, "/") + b;
}

Array.prototype.erase = function (a) {
	for (let i = this.length; i--;) this[i] === a && this.splice(i, 1);
	return this;
}
...
window.ig = {
	...
	nocache: '',
	ua: {},
	root: window.IG_ROOT || "/Impact/lib",
	...
	getCacheSuffix: function() {
		return ig.nocache ? "?nocache=" + ig.nocache : "";
	},
	
	addGameAddon: function(addon) {
		addons.push(addon)
	},

	initGameAddons: function() {
		let createdAddons = []
		for (let i = 0; i < addons.length; i++) {
			createdAddons.push(addons[i]());
		}
...
ig.Config = ig.Class.extend({
	_data: null,
	init: function(data) {
		for (let item in data) this[item] = data[item]
	},
	copy: function() {
		return new ig.Config(this._data)
	}
})
