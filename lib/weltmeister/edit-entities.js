...
wm.EditEntities = ig.Class.extend({
	...
	loadEntitySettings: function() {
		...
		if (this.selectedEntity._wm) {
			for (let i = 0; i < Object.entries(this.selectedEntity._wm._attributes).length; i++) {
				const [name, data] = Object.entries(this.selectedEntity._wm._attributes)[i];
				if (this.selectedEntity._wmSettings[name]) continue;

				html += $(`<div class="entityDefinition" title="${data._info}"><span class="key">${name}</span>:<span class="value">${data._default ? data._default : ""}</span></div>`)[0].outerHTML;
			}
		}
	...
	loadEntitySettingsRecursive: function( settings, path ) {
		var entity = this.entities.find(ent => ent._wmSettings === settings);
		if (!entity instanceof ig.Entity) entity = "" 

		path = path || "";
		var html = "";
		for( var key in settings ) {
			let docsKey = key.split(".")[0]
			var docs = entity ? entity._wm ? entity._wm._attributes[docsKey] : "" : "";
			...
			if( typeof(value) == 'object' ) {
				...
				html += `<div class="entityDefinition" ${docs ? `title="${docs._info}\nType: ${docs._type}"` : ""}><span class="key">${path}${key}</span>:<span class="value">${value}</span></div>`
			}
		}
